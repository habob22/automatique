...
def parse_file(file_path, last_position):
    """Parse the log file to extract packet counts, version, and rank."""
    counts = {'DIS': 0, 'DIO': 0, 'DAO': 0, 'DATA': 0}
    versions = set()
    ranks = set()
...
file_path = 'data/statistiques.txt'
last_position = 0

while True:
    counts, versions, ranks, features_list, last_position = parse_file(file_path, last_position)
